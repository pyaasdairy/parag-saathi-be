...
// and no dependencies outside the standard library. This is the contract
// layer: modules depend on domain, never the other way around.
package domain

// Role codes — the 23-role catalog from blueprint §5.2.
// A Party may hold many time-bounded, org-scoped RoleAssignments.
const (
	// Village tier
	RoleFarmer         = "FARMER"
	RoleSamitiSacheev  = "SAMITI_SACHEEV"  // DCS secretary — runs the collection console
	RoleSamitiAdhyaksh = "SAMITI_ADHYAKSH" // DCS chairman — approvals & governance co-sign
...
// AllRoles is the closed set of grantable role codes.
var AllRoles = []string{
...
// IsValidRole reports whether code is one of the 23 catalog roles.
...
// RequiredKYCTier maps each role to the minimum KYC tier demanded before the
// role assignment becomes usable (blueprint §5.2 "KYC tier" column).
