...
func TestDuplicateSubscriptionRefused(t *testing.T) {
	...
	// A label that is not one of the SKU's own priced variants is stored as
	// the SKU's size (lineVariant), so it is the same line: "1L" on the 500ml
	// SKU, or its size spaced differently, cannot open a second 500ml plan.
	for _, label := range []string{"1L", "500 ML"} {
		if _, err = w.svc.createSubscription(ctx, cid, subscriptionInput{ProductID: "gold-500ml", Variant: label, Qty: 1, Frequency: "daily"}); !errors.As(err, &ae) || ae.Code != "DUPLICATE_SUBSCRIPTION" {
			t.Fatalf("%q on the 500ml SKU must be the same line: %v", label, err)
		}
	}
	// A priced variant of the same product is its own line; another customer too.
	curd := 40.0
	if _, err := w.db.Collection(collCatalog).InsertOne(ctx, catalogDoc{
		SkuID: "curd-cup", Kind: catalogKindProduct, Price: &curd, Name: "Curd Cup", Category: "curd",
		Variants: []variantDoc{{VariantID: "200g", Label: "200g", Price: 40}, {VariantID: "400g", Label: "400g", Price: 75}},
	}); err != nil {
		t.Fatalf("seed a variant product: %v", err)
	}
	if _, err := w.svc.createSubscription(ctx, cid, subscriptionInput{ProductID: "curd-cup", Variant: "200g", Qty: 1, Frequency: "daily"}); err != nil {
		t.Fatalf("first variant: %v", err)
	}
	if _, err := w.svc.createSubscription(ctx, cid, subscriptionInput{ProductID: "curd-cup", Variant: "400g", Qty: 1, Frequency: "daily"}); err != nil {
		t.Fatalf("other variant: %v", err)
	}
	if _, err = w.svc.createSubscription(ctx, cid, subscriptionInput{ProductID: "curd-cup", Variant: "200G", Qty: 1, Frequency: "daily"}); !errors.As(err, &ae) || ae.Code != "DUPLICATE_SUBSCRIPTION" {
		t.Fatalf("the same priced variant twice: %v", err)
	}
