...
//
// Merged onto integration/delivery with three symbol adaptations and no
// change to any assertion: the collections are reached through the
// repository (repo.complaints(), repo.pushDevices()), and a complaint's
// Mongo id is MongoID (ID is the wire id, "cmp_...").
//
...
func TestPhase2ComplaintIsIdempotentByRef(t *testing.T) {
	...
	if _, err := w.svc.repo.complaints().UpdateOne(ctx,
		bson.D{{Key: "_id", Value: first.MongoID}},
...
func TestPhase2ErasureTakesComplaintsAndDevices(t *testing.T) {
	...
	if n, _ := w.svc.repo.complaints().CountDocuments(ctx,
		bson.D{{Key: "consumer_id", Value: cid}}); n != 0 {
		t.Fatalf("erased account left %d complaints behind", n)
	}
	if n, _ := w.svc.repo.pushDevices().CountDocuments(ctx,
		bson.D{{Key: "consumer_id", Value: cid}}); n != 0 {
...
func TestPhase2ComplaintIdempotentWithoutTheIndex(t *testing.T) {
	...
	if _, err := w.svc.repo.complaints().Indexes().DropAll(ctx); err != nil {
