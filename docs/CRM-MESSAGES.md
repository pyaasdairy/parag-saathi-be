...
| B-03 | `payment.failed` | 10 min after a Razorpay top-up payment fails (webhook, once per payment), skipped if a retry paid that same order meanwhile; or 10 min after an AutoPay (Smart Recharge) charge is refused (the bank refused it when it was made, or its payment.failed webhook; once per charge), skipped if that charge's order was captured meanwhile. A charge whose Razorpay order could not be created (a bad key, a gateway config error: nothing reached the bank) is our failure and sends nothing. The AutoPay one takes the AutoPay body, T-B03-AUTOPAY (condition `payment.source != 'autopay'` picks T-B03) | sms, +whatsapp, +push | DLT B-03; WA B-03 (both the checkout body T-B03 only); push. T-B03-AUTOPAY has no DLT or WhatsApp registration yet: inbox and push only | INBOX-ONLY (push once enabled) | Your recharge didn't go through. Try again or use another method — no money was deducted. / AutoPay (T-B03-AUTOPAY): AutoPay couldn't add ₹500 to your Wallet this time. Add money in the app so your next delivery isn't paused. ([X] is the charge's rupees; no claim about the bank account is made.) |
...
| B-04 | `0 10 * * *` | whatsapp | wallet plans have no renewal date (AutoPay tops the wallet up on need, not on a date) |
...
| AutoPay registration payment (`POST /mandate/verify`, or the webhook / reconcile sweep when the app never comes back) | B-06 (T-B05-REFUND) | exactly one: "₹500 added to your Wallet (AutoPay recharge). Refundable. …" | `TestAutopayRegistrationCreditsTheWalletOnce` |
| AutoPay Smart Recharge charge captured (webhook `payment.captured` / `order.paid`, or the reconcile sweep) | B-06 (T-B05-REFUND) | exactly one: "₹100 added to your Wallet (AutoPay recharge). Refundable." AutoPay FUNDS the wallet (founder, 25 Sep): it never debits it; a refused charge sends B-03 in AutoPay's words and credits nothing | case 8, `TestAutopaySmartRechargeCreditsOnceOnCapture`, `TestCRMEmitPaymentFailed` |
